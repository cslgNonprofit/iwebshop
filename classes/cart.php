...
class Cart
{
	...
	* [id]   :array  商品id值;
	* [count]:int    商品数量;
	* [info] :array  商品信息 ods]=>array( ['id']=>商品ID , ['data'] => array( [商品ID]=>array ( [sell_price]价格, [count]购物车中此商品的数量 ,[type]类型goods,product ,[goods_id]商品ID值 ) ) ) , [product]=>array( 同上 ) , [count]购物车商品和货品数量 , [sum]商品和货品总额 ;
	* [sum]  :int    商品总价格;
	*/
	...
	private function cartFormat($cartValue)
	{
		...
		if($goodsIdArray)
		{
			...
			$goodsObj   = new IModel('goods');
			$goodsData  = $goodsObj->query('id in ('.join(",",$goodsIdArray).')','id,name,list_img,sell_price');
			...
			{
				$result['goods']['data'][$key]['list_img']   = $goodsArray[$val['goods_id']]['list_img'];
			...
			{
				$result['product']['data'][$key]['list_img'] = $goodsArray[$val['goods_id']]['list_img'];
