    ...
    public function model_update($model_id,$model_name,$attribute,$spec)
    {
		...
		if($model_id)
		{
			...
			for( $i = 0;$i< $spec_len;$i++)
			{
				...
				$model_spec[$i]['name'] = $spec['name'][$i];
				$model_spec[$i]['show_type'] = $spec['show_type'][$i];
				$model_spec[$i]['note'] = $spec['note'][$i];
			}
			//序列化商品模型规格数据
			$spec_serialize = serialize($model_spec);
			$modelObj->setData(array('spec_ids'=>$spec_serialize));
			//更新商品模型规格
			$modelObj->update("id=$model_id");
			return true;
		}
...
/**
    ...
    public function get_model_info($model_id)
    {
		...
		{
			//反序列化 商品模型规格数据
			$spec_id_list = unserialize($model_info['spec_ids']);
			$model_info['model_spec'] = $spec_id_list;
			//初始化attribute商品模型属性表类对象
			$attributeObj = new IModel('attribute');
			//根据商品模型编号 获取商品模型扩展属性
