...
class pay_99bill extends paymentPlugin
{
    ...
    */
    public function toSubmit($payment)
    {
		...
		$merchantAcctId=$this->getConf($payment['M_Paymentid'], 'merchant_id');

		//人民币网关密钥
		///区分大小写.请与快钱联系索取
		$key=$this->getConf($payment['M_Paymentid'], 'merchant_key');//商户密钥
    ...
    function callback($in,&$paymentId,&$money,&$message,&$tradeno)
    {
		...
		//设置人民币网关密钥
		///区分大小写
		$key=$this->getConf($paymentId, 'merchant_key');//商户密钥
